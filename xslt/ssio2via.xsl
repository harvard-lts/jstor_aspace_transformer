...
<!--
   This version of ssio2via is for regular nightly harvest, 
   as opposed to ssio2viafull.xsl, which is used for full harvests. 
   The difference is that the former will look up group recs for recs 
        ...
        <xsl:choose>
            <xsl:when test="$displaycount = $deletecount">
                <xsl:variable name="deletepath">
                    <xsl:value-of select="$harvestdir"/>/DELETES/<xsl:value-of select="ssw:Work/@id"
            ...
            <xsl:otherwise>
                <xsl:if
                    test="ssw:Work/RelatedWorks/ssw:RelatedWork[lower-case(@type_lkup) = 'part of']">
                    <xsl:variable name="filename">
                        <xsl:value-of
                            select="ssw:Work/RelatedWorks/ssw:RelatedWork[lower-case(@type_lkup) = 'part of']/@refid"/>
                        <xsl:text>.xml</xsl:text>
                    </xsl:variable>
                    <xsl:variable name="fullpath">
                        <xsl:value-of select="$harvestdir"/>/LARGERCONTEXT/<xsl:value-of
                            select="$filename"/>
                    </xsl:variable>
                    <xsl:result-document href="{$fullpath}">
                        <xsl:element name="viaRecord">
                            ...
                            <xsl:apply-templates
                                ...
                                mode="primaryThumb"/>
                            <xsl:apply-templates
                                select="ssw:Work/RelatedWorks[ssw:RelatedWork/@type_lkup = 'Part of']"/>
                            <xsl:apply-templates
                                select="ssw:Work/RelatedWorks[ssw:RelatedWork/@type_lkup = 'part of']"/>
